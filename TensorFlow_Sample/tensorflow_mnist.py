...
NUM_LABELS = 10
# 创建变量，包含所有可训练的权重
conv1_weights = tf.Variable(tf.truncated_normal([5, 5, NUM_CHANNELS, 32], stddev=0.1, dtype=tf.float32), name='Conv1_weights')
...
fc1_weights = tf.Variable(tf.truncated_normal([IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * 64, 512], stddev=0.1, dtype=tf.float32), name='Fc1_weights')
...
# 样本损失
loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels_placeholder, logits=model_output), name='Loss')
# L2 正则化损失
...
# 学习率是从 0.01 开始的指数衰减
learning_rate = tf.train.exponential_decay(0.01, learning_rate_batch * BATCH_SIZE, train_size, 0.95, staircase=True, name='Learning_rate')
...
for step in range(generations):
    ...
    if (step+1)%EVAL_FREQUENCY == 0 or (step+1) == generations:
        ...
        validation_error = 100.0 - (100.0*np.sum(np.argmax(predictions_eval,1) == validation_labels)/predictions_eval.shape[0])
        # 更新汇总数据
        ...
        print('\nStep #%d of %d (epoch %.2f of %d): Average time = %.1f ms' % 
                (step+1, generations, float(step)*BATCH_SIZE/train_size, NUM_EPOCHS, 1000*elapsed_time/EVAL_FREQUENCY))
